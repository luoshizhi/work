...
Go: while (<BLOCK>) {
	...
	if (@array<1){
		...
		for (my $i=0;$i<@array;$i++) {
			my @list=@{$array[$i]};
			my $lc = List::Compare->new(\@list, \@block_temp);
		...
		for (my $i=0;$i<@block_temp;$i++) {
			if (exists $list{$block_temp[$i]}) {
				push @array,[@block_temp];
				next Go;
			}
		}
...
#	next if (/At\d+&Atd+/);
	...
	my($region_1_star,$region_1_end)=&range_region(@region_1);
	my($region_2_star,$region_2_end)=&range_region(@region_2);
	my @target1;my @target2;
	for (my $i=0;$i<@{$block{$gene1}};$i++) {
		if (exists $list{$block{$gene1}->[$i]}) {
			my $target_gene1=$block{$gene1}->[$i];
			if ($pos{$target_gene1}->[1]>$region_1_star && $pos{$target_gene1}->[2]>$region_1_end) {
	...
	for (my $i=0;$i<@{$block{$gene2}};$i++) {
		if (exists $list{$block{$gene2}->[$i]}) {
			...
			if ($pos{$target_gene2}->[1]>$region_2_star && $pos{$target_gene2}->[2]>$region_2_end) {
	...
	if (@target1>=1 && @target2>=1) {
		print "chr\t$chr1,$chr2\n";
		print "region\t$chr1\t$region_1_star\t$region_1_end\t$chr2\t$region_2_star\t$region_2_end\n";
		foreach my $id1 (@target1) {
			print "pos\t$pos{$id1}->[0]\t$pos{$id1}->[1]\t$pos{$id1}->[2]\t$id1\n";
		}
		foreach my $id2 (@target2) {
			print "pos\t$pos{$id2}->[0]\t$pos{$id2}->[1]\t$pos{$id2}->[2]\t$id2\n";
		}
	}
