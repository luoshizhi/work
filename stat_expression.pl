...
my $file = `find  $dir -name *DEG*final.xls `;

my @file=split/\n/,$file;
my %Stat;
foreach my $DEG (@file) {
	my $file_name=basename($DEG);
	my($name)=$file_name=~/(\w+)/;
	open (IN,$DEG) or die $!;
	while (<IN>) {
		chomp;
		next if /^\#/;
		my $type=(split/\s+/,$_)[-1];
		$Stat{$name}{up}++ if $type eq 'up';
		$Stat{$name}{down}++ if $type eq 'down';
		$Stat{$name}{total}++;
	}
}
open (OUT,">>$out")||die $!;
print OUT "#Group\tAll_DEG\tup-regulated\tdown-regulated\n";
foreach my $name (sort keys %Stat) {
	$Stat{$name}{up} ||=0;
	$Stat{$name}{down} ||=0;
	print OUT "$name\t$Stat{$name}{total}\t$Stat{$name}{up}\t$Stat{$name}{down}\n";
}
	close IN;
