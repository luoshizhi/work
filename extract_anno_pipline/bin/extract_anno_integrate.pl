...
my %opts;
GetOptions(\%opts,"gene=s","id=s","od=s","key=s","list=s","h");
if (!defined($opts{gene}) || !defined($opts{id}) || !defined($opts{od}) || !defined($opts{key}) || defined($opts{h}) ||!defined($opts{list})) {
	&help;
	exit;
}
