...
foreach my $c_file (@cDNA) {
	...
	while (<cDNA>){
		...
		if (/target\s/) {
			my ($MRE)=/5'\s*([A-Za-z-]+)/;
			$miRNA_cDNA{$miRNA_temp}{$cDNA_temp}="$MRE $range";
		}
	}
}
close cDNA;
...
foreach my $lnc_file (@lncRNA) {
	...
	while (<Lnc>){
		...
		if (/target\s/) {
			my ($MRE)=/5'\s*([A-Za-z-]+)/;
			$miRNA_lncRNA{$miRNA_temp}{$lncRNA_temp}="$MRE $range";
		}
	}
	close Lnc;
}
...
foreach my $miRNA(sort keys %miRNA_lncRNA){
	...
	if (exists $miRNA_cDNA{$miRNA}) {
		foreach my $cDNA1(sort keys %{$miRNA_cDNA{$miRNA}}){
			$join .="$space\| --> cDNA:$cDNA1($miRNA_cDNA{$miRNA}{$cDNA1})\n";
		}
	}
	foreach my $lncRNA1(sort keys %{$miRNA_lncRNA{$miRNA}}){
		if (exists $lncRNA_target{$lncRNA1}) {
			$join .="$space\| --> lncRNA:$lncRNA1($miRNA_lncRNA{$miRNA}{$lncRNA1}) --> $lncRNA_target{$lncRNA1}\n";
		}else{
			$join .="$space\| --> nlcRNA:$lncRNA1\n";
		}

	}
	open (OUT,">$od_miRNA/${miRNA}_target_net.txt") or die $!;
	print OUT "#MiRNA  -->  cDNA/lncRNA(MRE range)  -->  lncRNA_target\n";
	print OUT $join;
}
...
foreach my $lncRNA1(sort keys %lncRNA_miRNA ){
	...
	foreach my $miRNA(sort keys %{$lncRNA_miRNA{$lncRNA1}}){
		...
		if (exists $lncRNA_target{$lncRNA1}) {
			$join .="$miRNA$space --> $lncRNA1($miRNA_lncRNA{$miRNA}{$lncRNA1}) --> $lncRNA_target{$lncRNA1}\n";
		}else{
			$join .="$miRNA$space --> $lncRNA1($miRNA_lncRNA{$miRNA}{$lncRNA1})\n";
		}
	}
	open (OUT,">$od_lncRNA/${lncRNA1}_net.txt") or die $!;
	print OUT "#MiRNA  -->  lncRNA(MRE range)  -->  lncRNA_target\n";
	print OUT $join;
}
